...
(function () {
  function setOpenState(item, open) {
    ...
    if (open) {
      // Перед открытием сбросим, затем поставим реальную высоту
      content.style.maxHeight = 'none';
      const target = content.scrollHeight;
      // Сначала 0 → чтобы анимация сработала
      content.style.maxHeight = '0px';
      // Следующий тик — к целевой высоте
      ...
      // Зафиксируем текущую высоту, затем анимируем к 0
  ...
  function init(container) {
    const items = (container || document).querySelectorAll('.faq-item');
    items.forEach((item) => {
  ...
  // Пересчёт высоты при ресайзе (если контент изменил высоту)
  window.addEventListener('resize', () => {
    document.querySelectorAll('.faq-item.faq-active .faq-content').forEach((c) => {
      c.style.maxHeight = c.scrollHeight + 'px';
    });
  });

  // Если используется фронт-редактирование django CMS, ловим обновления DOM
